...
const Manage = () => {
    ...
    let apiRoute = router.query.query 
        ? `/api/dvds/search?q=${router.query.query}`
        : `/api/dvds?limit=20&skip=${skip}`
    ...
    useEffect(() => {
      ...
      if (!data && router.isReady) getDvds()
    })
    ...
    if (!data?.dvds?.length) return <Loading/>
    
    const deleteDvd = async (dvd: any) => {
        await axios.post('/api/dvds/delete', dvd)
        setDeleting(null)
    }
    ...
    return (
      <Layout>
        ...
        <div className='container mx-auto bg-white rounded dark:bg-zinc-800 dark:text-white pt-5'>
            {data.dvds.length > 0 && <>
              <h1 className='px-6 mb-3 sm:px-20 md:px-10 lg:px-5 text-3xl pb-2 mx-auto'>Manage DVDs</h1>
              {
                page && <div className='px-6 mb-3 sm:px-20 md:px-10 lg:px-5 flex flex-row items-center'>
                    { page > 1 && <PageNavigation href={`/dvds/manage?page=${1}`} name='first_page'/> }
                    { page > 1 && <PageNavigation href={`/dvds/manage?page=${page-1}`} name='navigate_before'/> }
                    <span>{`Page ${page}/${totalPages}`}</span>
                    { page < totalPages && <PageNavigation href={`/dvds/manage?page=${page+1}`} name='navigate_next'/> }
                    { page < totalPages && <PageNavigation href={`/dvds/manage?page=${totalPages}`} name='last_page'/> }
              </div>
              }
              ...
              <table className='table-auto w-full text-left border-zinc-300 dark:border-zinc-700'>
                <thead>
                    <tr>
                        <th className='border-b border-zinc-300 dark:border-zinc-700 px-4 text-center'>DVD</th>
                ...
                <tbody>
                    { data.dvds?.map((dvd: any, i: number) => 
                        <tr key={i}>
                            ...
                            <td className='px-3 border border-zinc-300 dark:border-zinc-700'>
                                <a href={`/dvds/edit?v=${dvd.id}`}><span>{dvd.title}</span></a>
                            </td>
                            <td className='text-center border border-zinc-300 dark:border-zinc-700'>
                                <a href={`/dvds/edit?v=${dvd.id}`}><Icon name='edit'/></a>
                            </td>
                            <td className='text-center border border-zinc-300 dark:border-zinc-700'>
                                <Icon name='delete' video={JSON.stringify(dvd)} className='cursor-pointer' onClick={(e: any) => {
