...
const Manage = () => {
    ...
    let apiRoute = router.query.query 
        ? `/api/playlists/search?q=${router.query.query}`
        : `/api/playlists?limit=20&skip=${skip}`
    ...
    useEffect(() => {
      ...
      if (!data && router.isReady) getPlaylists()
    })
    ...
    if (!data?.playlists?.length) return <Loading/>
    
    const deletePlaylist = async (playlist: any) => {
        await axios.post('/api/playlists/delete', playlist)
        setDeleting(null)
    }
      ...
      <Layout>
            ...
            <Modal buttons={[
                () => (
                    <button type="button"
                        ...
                        onClick={() => deletePlaylist(deleting)}
                    >
                        Delete
                    </button>
                ),
        ...
        <div className='container mx-auto bg-white rounded dark:bg-zinc-800 dark:text-white pt-5'>
            {data.playlists.length > 0 && <>
              <h1 className='px-6 mb-3 sm:px-20 md:px-10 lg:px-5 text-3xl pb-2 mx-auto'>Manage playlists</h1>
              {
                page && <div className='px-6 mb-3 sm:px-20 md:px-10 lg:px-5 flex flex-row items-center'>
                    { page > 1 && <PageNavigation href={`/playlists/manage?page=${1}`} name='first_page'/> }
                    { page > 1 && <PageNavigation href={`/playlists/manage?page=${page-1}`} name='navigate_before'/> }
                    <span>{`Page ${page}/${totalPages}`}</span>
                    { page < totalPages && <PageNavigation href={`/playlists/manage?page=${page+1}`} name='navigate_next'/> }
                    { page < totalPages && <PageNavigation href={`/playlists/manage?page=${totalPages}`} name='last_page'/> }
              </div>
              }
              ...
              <table className='table-auto w-full text-left border-zinc-300 dark:border-zinc-700'>
                <thead>
                    <tr>
                        <th className='border-b border-zinc-300 dark:border-zinc-700 px-4 text-center'>Playlist</th>
                    ...
                    { data.playlists?.map((playlist: any, i: number) => 
                        <tr key={i}>
                            <td className='my-2  w-32 h-16 aspect-video relative'>
                                <Image 
				                    layout='fill'
				                    src={`${appConfig.publicThumbnails}${playlist.videos[0].id}.png`} 
                                    alt="" />
                            </td>
                            <td className='px-3 border border-zinc-300 dark:border-zinc-700'>
                                <a href={`/playlists/edit?v=${playlist.id}`}><span>{playlist.title}</span></a>
                            </td>
                            <td className='text-center border border-zinc-300 dark:border-zinc-700'>
                                <a href={`/playlists/edit?v=${playlist.id}`}><Icon name='edit'/></a>
                            </td>
                            <td className='text-center border border-zinc-300 dark:border-zinc-700'>
                                <Icon name='delete' video={JSON.stringify(playlist)} className='cursor-pointer' onClick={(e: any) => {
