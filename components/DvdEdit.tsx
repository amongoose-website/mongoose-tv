...
const DvdEdit = ({dvd}: {dvd: any}) => {
    const [formData, setFormData] = useState(dvd)
    const [submitted, setSubmitted] = useState(false)
    if (formData.dvdNumber && formData.episodeNumber && !dvd.isDvd) formData.isDvd = true
    ...
    const handleSubmit = async (event: any) => {
        ...
        const { data }= await axios.post(`/api/dvds/edit`, formData)
        setSubmitted(true)
    }
                                        ...
                                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div className="mt-2">
                                                <p className="text-sm text-gray-500">
                                                    <span>The DVD has been updated and is accessible.</span>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                ...
                <form method='post' onSubmit={handleSubmit}>
                    <div className="flex flex-col justify-center md:w-min min-w-fit gap-2">
                        {dvd && <>
                            ...
                            <textarea name="description" id="description" placeholder='Enter a description..' defaultValue={dvd.description}
                    ...
                    <div className="flex flex-row items-center gap-2">
                        <button type='submit' className='text-white bg-blue-500 border border-blue-500 rounded px-5 py-2 w-fit'>Save Dvd</button>
                        <input
                            type='button'
                            value='Reset Changes'
                            className='cursor-pointer dark:border-white dark:text-white border border-zinc-700 text-zinc-700 rounded px-5 py-2 w-fit'
                            onClick={() => setFormData(dvd)}/>
                    </div>
                </div>
            </form>
        </div>
