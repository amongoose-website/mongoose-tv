...
const SearchBar = () => {
    return <form method='get' action='/results'>
        <input 
            id="search"
            autoCapitalize="none"
            autoComplete="off"
            ...
            dir="ltr"
            className='block p-2 pl-10 w-full text-zinc-900 bg-zinc-50 rounded-lg border border-zinc-300 sm:text-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:border-zinc-600 dark:placeholder-zinc-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 outline-none'
    />
</form>
}
        ...
        <nav className="bg-white border-zinc-200 my-2 px-2 sm:px-4 py-3 rounded dark:bg-zinc-800 container mx-auto">
            <div className="container flex md:gap-10 flex-wrap justify-between items-center mx-auto">
                ...
                <div className="flex flex-grow justify-end">
                    ...
                    <button id="searchButton" type="button" data-collapse-toggle="mobile-menu-3" aria-controls="mobile-menu-3"
                        aria-expanded="false"
                        ...
                        onClick={() => setMenu(menu !== 'openSearch' ? 'openSearch' : 'closed')}>
                    ...
                    <button id='hamburgerButton' data-collapse-toggle="mobile-menu-3" type="button"
                        ...
                        onClick={() => setMenu(menu !== 'openMenu' ? 'openMenu' : 'closed')}>
                ...
                <div className={`justify-between items-center w-full md:flex md:w-auto md:order-1`} id="mobile-menu-3">
                    <div id='searchBar' className={`relative mt-3 md:hidden ${menu === 'openSearch' ? '' : 'hidden'}`}>
                        <div className="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                            <svg className="w-5 h-5 text-zinc-500" fill="currentColor" viewBox="0 0 20 20"
                                xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd"
                    ...
                    <ul className={`${menu === 'openMenu' ? 'flex' : 'hidden'} md:flex flex-col items-center gap-2 mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium text-center`}>
                        ...
                        {user && <>
                            ...
                            <div className={`${userMenu ? 'md:visible' : ''} rounded flex flex-col invisible absolute px-5 right-0 z-30 bg-white dark:bg-zinc-800 w-72 py-5 border-solid dark:border-zinc-700 dark:text-white border-zinc-200 border`}
                                style={{top: '85.5px'}}>
                                <ul>
